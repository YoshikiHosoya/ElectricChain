...
#define WINDOW_MODE		(FALSE)					//ウィンドウモードかどうか (FALSEでフルスクリーン）
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hInstancePrev, LPSTR lpCmdLine, int CmdShow)
{
	...
	WNDCLASSEX wcex =
	{
		...
		LoadIcon(hInstance,MAKEINTRESOURCE(IDI_TEST)),		//タスクバーに使用者のアイコン設定
		LoadCursor(NULL,IDC_ARROW),							//使用するマウスカーソル指定
		(HBRUSH)(COLOR_WINDOW + 1),							//ウィンドウクライアント領域の色指定
		NULL,												//メニューを指定
		CLASS_NAME,											//ウィンドウクラスの名前
		LoadIcon(NULL,MAKEINTRESOURCE(IDI_TEST))			//小さいアイコンの指定
	};
